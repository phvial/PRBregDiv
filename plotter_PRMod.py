"""
Evaluation and figures for phase retrieval from modified spectrograms
...
def main(argv):
    ...
    for i in range(n_files):
        ...
        for j in range(len(GD_algs)):
            j_gd = GD_algs[j]
            x_gen, sr = l.load(sig_path+'%i_%s_GD_%s_%s_d%i.wav'%(i, snrdB, j_gd[0], j_gd[1], j_gd[3]))
            distmat[i,j,:] = measuresDiv(original, x_gen, divs)
        
        for j in range(len(ADMM_algs)):
            j_admm = ADMM_algs[j]
            x_gen, sr = l.load(sig_path+'%i_%s_ADMM_%s_%s_d%i.wav'%(i, snrdB, j_admm[0], j_admm[1], j_admm[2]))
            distmat[i,j+len(GD_algs),:] = measuresDiv(original, x_gen, divs)
            
        for j in range(len(other_algs)):
            j_other = other_algs[j]
            x_gen, sr = l.load(sig_path+'%i_%s_%s.wav'%(i, snrdB, j_other))
